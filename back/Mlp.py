...
def organizar_dataset(json_positivo, json_negativo, quant, fold):
    ...
    frequencia = TfidfTransformer()
    X_treino = frequencia.fit_transform(X_treino)
    
    mlp = MLPClassifier()
    
    mlp.fit(X_treino, label_treino)
    
    #testando abaixo
    X_teste = vectorizer.transform(reviews_test)
    X_teste = frequencia.transform(X_teste)
    predito = mlp.predict(X_teste)
